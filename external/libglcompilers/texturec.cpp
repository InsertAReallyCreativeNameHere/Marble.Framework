...
std::vector<char> compileTexture(const std::vector<char>& textureData, const std::vector<const char*>& args)
{
	...
	bimg::ImageContainer* output = convert(&allocator, textureData.data(), textureData.size(), options, &err);
	...
	if (NULL != output)
	{
		...
		{
			bimg::imageWriteKtx(&memWriter, *output, output->m_data, output->m_size, &err);
		}
		else if (!bx::strFindI(saveAs, "dds").isEmpty() )
		{
			bimg::imageWriteDds(&memWriter, *output, output->m_data, output->m_size, &err);
		}
		...
		if (validate)
		{
			bimg::ImageContainer* input = bimg::imageParse(&allocator, textureData.data(), textureData.size(), bimg::TextureFormat::Count, &err);
		...
		std::vector<char> ret(memWriter.internalBuffer.begin(), memWriter.internalBuffer.end());
		bimg::imageFree(output);
		return std::move(ret);
	}
	else
